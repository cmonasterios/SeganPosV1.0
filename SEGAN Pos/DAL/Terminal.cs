                ...
                if (_Modelo == 1)
                {
                                ...
                                out TMinutos, out TSegundos, ref TCodigoTrabajo))
                    {
                        ...
                        if (LecturaActual.CompareTo(_FechaUltAct) > 0)
                        {
                            ...
                            int result = Reporte.EPK_LecturasActualizar(_IdRegion, _IdLocalidad, Convert.ToInt32(TIdenUsuario), _IdTerminal, FechaHora.ToString("yyyymmdd"), FechaHora.ToString("HH:mm:ss"), Convert.ToInt16(TBanderaEntrada), 0, Convert.ToInt16(TModoVerificacionAcceso), Convert.ToInt16(TCodigoTrabajo));
                        }
                    }
                }
                else
